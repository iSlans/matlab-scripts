function [x, y] = primalSimplex(c, A, b, baseIndex, opt)
    ...
    if all(direction_vector <= 0)
        log.info("optimun value is Infinity \n")
        return
    end
    ...
    x = primalSimplex(c, A, b, newBase, currentDepth = opt.currentDepth + 1, depthLimit = opt.depthLimit);
end
